...
function createAnimatedComponent(Component: any): any {
  ...
  class AnimatedComponent extends React.Component<Object> {
    ...
    _reattachNativeEvents(prevProps) {
      ...
      // Make sure events that were in prev and next props have same config.
      for (const key of attached) {
        if (nextEvts.has(key)) {
          const attachedEvent = prevProps[key];
          const nextEvent = this.props[key];
          if (!areEqual(attachedEvent._argMapping, nextEvent._argMapping)) {
            attachedEvent.__detach(node, key);
            nextEvent.__attach(node, key);
          }
        }
      }
    }
