...
const _require3 = require('./errors.js'),
  ...
  ModuleFlowInterfaceNotParserError =
    _require3.ModuleFlowInterfaceNotParserError,
...
function buildModuleSchema(
  ...
  const moduleInterfaceNames = Object.keys(types).filter(typeName => {
  ...
  const _moduleInterfaceNames = _slicedToArray(moduleInterfaceNames, 1),
    moduleInterfaceName = _moduleInterfaceNames[0];
  ...
  return declaration.body.properties
    ...
    .filter(Boolean)
    .reduce(
      (moduleSchema, {aliasMap, propertySchema}) => {
        return {
          type: 'NativeModule',
